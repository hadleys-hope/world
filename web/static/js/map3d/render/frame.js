...
import { updateCameraMotion } from '../camera.js';
...
export function frame() {
  ...
  if (state.flyAnim) {
    ...
    state.controls.target.lerpVectors(state.flyAnim.tfrom, state.flyAnim.tto, k);
  ...
  updateUtilityFlow(t);
  {
    const hour = (state.clock.hour + state.clock.speed * (now - state.clock.at) / 1000 / 60) % 24;
    const a = (hour - 6) / 24 * Math.PI * 2;
    state.sunDir.set(Math.cos(a) * 12000, 3200, Math.sin(a) * 12000).normalize();
    state.sun.position.copy(state.sunDir).multiplyScalar(12000);
    state.sunSprite.position.copy(state.sunDir).multiplyScalar(48000);
    const dl = Math.max(0, Math.sin(Math.PI * (hour - 6) / 12)) * (state.weather.storm ? 0.2 : 1);
    ...
    const wantFog = state.weather.storm ? 0.00055 : state.weather.precip === 'snow' ? 0.00018 : 0.0;
    ...
    const snowOn = state.weather.precip === 'snow' || state.weather.storm;
    ...
    mat.opacity += ((snowOn ? (state.weather.storm ? 0.85 : 0.55) * near : 0) - mat.opacity) * 0.05;
    mat.size = 3 + 3 * near;
    if (mat.opacity > 0.02) {
      ...
      state.weather.snow.quaternion.copy(new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), n));
    }
    if (state.weather.tornadoOn) {
  ...
  if (state.housesMesh) {
    ...
    for (const g of state.gates) {
      if (g.armTarget !== undefined) g.arm.rotation.x += (g.armTarget - g.arm.rotation.x) * 0.15;
      g.beacons.forEach((b, i) => {
        b.material.opacity = 0.3 + 0.7 * Math.max(0, Math.sin(t * 8 + i * Math.PI));
      });
    }
    for (const w of state.lockWedges) {
      if (w.visible) w.material.opacity = 0.10 + 0.08 * Math.sin(t * 3);
    }
    for (const p of state.xenoPool) {
      if (p.to) {
        ...
        const moving = p.state === 'hunt' || p.state === 'approach' || p.state === 'retreat';
        p.g.children[0].position.y = 4 + (moving ? Math.abs(Math.sin(t * 14)) * 1.2 : 0);
        if (p.state === 'attack') p.g.children[1].position.z = 5.5 + Math.sin(t * 20) * 1.5;
        if (p.state === 'dying') p.g.rotation.z = Math.min(1.4, p.g.rotation.z + 0.1);else p.g.rotation.z = 0;
      }
    }
    ...
    for (const rv of Object.values(state.rovers)) {
      if (rv.to) {
        ...
        for (const wheel of rv.g.userData.wheels || []) wheel.rotation.z = -rv.distance / .66;
      }
    }
    ...
    if (state.complex.wheel) state.complex.wheel.rotation.z += state.S && state.S.power.mine ? 0.05 : 0;
    if (state.complex.steamOn !== undefined) {
      const c = state.G.cfg;
      const pts = [];
      for (let k = 0; k < 12; k++) {
        const u = (t * 0.25 + k / 12) % 1;
        for (const [tx, ty] of [[c.reactor_pos[0] - 90, c.reactor_pos[1] - 60], [c.reactor_pos[0] - 90, c.reactor_pos[1] + 60]]) pts.push(sph(tx + Math.sin(k * 3 + t) * 6 * u, ty + Math.cos(k * 2) * 6 * u, 95 + u * 60));
      }
      const a = state.complex.steam.geometry.attributes.position;
      for (let i = 0; i < a.count; i++) {
        if (state.complex.steamOn && i < pts.length) a.setXYZ(i, pts[i].x, pts[i].y, pts[i].z);else a.setXYZ(i, 0, -99999, 0);
      }
      a.needsUpdate = true;
    }
  }
  if (state.housesMesh) {
    for (const m of state.pipeMats) {
      if (m.userData.rate > 0) m.userData.tex.offset.x -= m.userData.rate * 0.02;
    }
    if (state.complex.beacon) state.complex.beacon.material.opacity = 0.4 + 0.6 * Math.abs(Math.sin(t * 2));
  }
  state.sunTarget.position.copy(state.controls.target);
  const sd = state.camera.position.distanceTo(state.controls.target);
  const desiredNear = Math.max(.1, Math.min(20, sd * .002));
  if (Math.abs(state.camera.near - desiredNear) > .01) {
  ...
  state.sun.position.copy(state.controls.target).add(state.sunDir.clone().multiplyScalar(8000));
  if (state.layers.bloom) state.composer.render();else state.renderer.render(state.scene, state.camera);
