...
export function terrainH(x, y) {
  ...
  const T = state.TER;
  let h;
  if (r < T.hub) h = 8.0;else if (r < T.row0 - 30) h = 8.0 - 3.0 * state.sstep(T.hub, T.row0 - 30, r); // hub on a mound, sloping to the first street
  else if (r < T.ring) {
  ...
  else if (r < T.wall + 20) h = 5.0 + T.rows * 3.4;else h = (5.0 + T.rows * 3.4) * (1 - state.sstep(T.wall + 20, T.wall + 260, r)); // outside the wall the ground eases down
  let natural = 22 + 24 * Math.sin(x * .0018) * Math.cos(y * .0024) + 14 * Math.sin(x * .004 + y * .001);
  for (const [px, py, height, width] of [[1250, 850, 180, 280], [-450, -1350, 145, 360], [-1640, 980, 220, 310], [450, 1650, 190, 330]]) natural += height * Math.exp(-((x - px) ** 2 + (y - py) ** 2) / (width * width));
  return coastHeight(x, y, h + state.sstep(T.wall + 20, T.wall + 320, r) * natural + 0.5 * Math.sin(x * 0.031) * Math.cos(y * 0.027));
}
export function sph(x, y, h = 0) {
  ...
  return new THREE.Vector3(r * Math.sin(th) * Math.cos(ph), r * Math.cos(th), r * Math.sin(th) * Math.sin(ph));
}
