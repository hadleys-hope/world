...
export async function checkToken() {
  ...
  state.tokenState.textContent = !r.protected ? 'open server' : r.ok ? 'control enabled' : 'view only';
...
export function selectPanel(name) {
  ...
  document.querySelectorAll('.hud-panel').forEach(p => p.hidden = p.id !== 'panel-' + name);
}
export function updateMapKey() {
  const items = [];
  const add = (color, label) => items.push(`<span><i style="--swatch:${color}"></i>${label}</span>`);
  ...
  if (!state.layers.underground && !state.layers.power && !state.layers.packets) {
    add('#a7af94', 'Habitat');
    add('#ffd18c', 'Lighting');
  }
...
export async function poll() {
  try {
    ...
    document.getElementById('banner').textContent = 'update failed: ' + (e && e.message || e);
  }
  setTimeout(poll, 300);
}
export function initialize() {
  state.ADMIN = new URLSearchParams(location.search).get('admin') || localStorage.getItem('hh_admin') || '';
  ...
  document.getElementById('pause').onclick = () => state.post({
    cmd: 'pause'
  });
  ...
  state.sliderToSpeed = v => v === 0 ? 0 : Math.round(Math.exp(Math.log(600) * v / 100));
  state.speedToSlider = s => s <= 0 ? 0 : Math.round(Math.log(Math.max(1, s)) / Math.log(600) * 100);
  state.speedEl.oninput = () => {
    state.speedV.textContent = state.sliderToSpeed(+state.speedEl.value) + ' min/s';
  };
  state.speedEl.onchange = () => state.post({
    cmd: 'speed',
  ...
  document.querySelectorAll('[data-view]').forEach(b => b.onclick = () => {
    ...
    document.querySelectorAll('#layers input').forEach(c => c.checked = !!state.layers[c.dataset.l]);
    document.querySelectorAll('[data-view]').forEach(t => t.setAttribute('aria-pressed', String(t === b)));
    applyLayers();
  });
  document.querySelectorAll('#layers input').forEach(c => c.addEventListener('change', () => {
  ...
  document.addEventListener('keydown', e => {
    ...
    if ('1234'.includes(k) && k.length === 1) document.querySelectorAll('[data-view]')[+k - 1].click();
  });
